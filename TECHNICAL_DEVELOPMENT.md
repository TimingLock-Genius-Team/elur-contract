...
当前实现采用直接部署架构，不依赖外部 `sat1HookDeployer`。如果后续合约工厂改为复用 sat1 的 Hook deployer，已确认的 Ethereum mainnet `Sat1HookDeployer` 参考地址是 `0xcbE096C140dB48199CC7e481116FD835BC33eDC6`；该合约部署出的 sat1 Hook 为 `0x2a0A30dd78aF7698E6f40212b8B8324fcE2ee888`，sat1 Token 为 `0x8f66337a0c2A02202fd91Dd596c411CF977c6060`。复用前必须为目标链重新确认 deployer code、源码、salt / init code 计算、Hook 权限位和 Factory 构造依赖，不能把 Ethereum 地址直接当作 XLayer 生产地址。
...
- 将 `BaseUniswapV4MigrationTarget` 外壳补全为真实 Uniswap v4 + LP burn/lock 适配器。
