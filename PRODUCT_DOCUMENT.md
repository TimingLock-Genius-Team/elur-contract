...
Eulr 是运行在 XLayer 主网的无许可代币发射协议。任何用户都可以通过链上工厂创建一个使用 sat1 bonding curve 机制的 ERC-20 代币。
...
这些客户端和数据 API 虽不在本仓库实现范围内，但本文与 `FRONTEND_INTEGRATION.md` / `TECHNICAL_DEVELOPMENT.md` 会约定它们依赖的链上事件、只读接口和 Bun.js backend/indexer 数据边界。
...
- 调用 `EulrFactory.createToken` 创建新代币。
...
- Hook 曲线储备有足够 OKB 覆盖 `grossOkbOut`，其中 `netOkbOut` 支付给 recipient，fee 留在 `claimableFeeOkb`。
...
```solidity
event TokenCreated(address indexed token, address indexed hook, address router, address indexed creator, string metadataURI, string socialURI);
...
`LiquidityMigrationResult` 只表示 migration target 返回的 pool / venue address 和 liquidity，不作为 LP 已 burn/lock 的证明；Uniswap v4 pool 身份应以 `PoolId` 表示，当前 `UniswapV4MintPositionTarget` 会 emit `UniswapV4PositionMinted` 和 `LpCustodyProven`，真实 XLayer fork 必须证明 position owner 最终是 burn / lock recipient。

前端和第三方客户端可使用 `EulrFactory.getTokens(offset, limit)` 分页读取已创建 token，并使用 `EulrHook.curveState()` 一次性读取 token 详情页所需的曲线位置、价格、fee、毕业和迁移状态。

详情页图表的数据来源分层：当前价格、supply、reserve 和进度以链上 `curveState()` 为准；理论 price curve 和 issuance curve 由前端用 `K` / `S` 本地采样计算；历史价格、24h volume、price change、sparkline、holders、trades 和 portfolio cost basis 由独立 Bun.js backend/indexer 从 `TokenCreated`、`Bought`、`Sold` 和 ERC-20 `Transfer` 事件聚合。当前合约事件已覆盖这些图表和聚合字段，不需要为该详情页设计新增合约状态。
...
- `UniswapV4MintPositionTarget` 第一版和 migration target deployment doctor。

仍不满足主网商业可用：

- 真实 XLayer 外部地址未全部确认。
- 真实 XLayer PositionManager 行为、PoolId 和 LP burn/lock 归宿未在 fork 中证明。
