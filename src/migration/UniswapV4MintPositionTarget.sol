...
interface IERC20ApproveBalance {
...
contract UniswapV4MintPositionTarget is BaseUniswapV4MigrationTarget {
    ...
    function _migrateValidated(
        ...
        if (!IERC20ApproveBalance(token).approve(positionManager, tokenAmount)) revert TokenApprovalFailed();
        posm.modifyLiquidities{value: okbAmount}(_encodeModifyLiquidities(key, params), params.deadline);
        if (!IERC20ApproveBalance(token).approve(positionManager, 0)) revert TokenApprovalFailed();

        liquidity = posm.getPositionLiquidity(positionId);
        if (IERC20ApproveBalance(token).balanceOf(address(this)) != 0) revert ResidualToken();
